...
For dispatch, failure handling, and local advisory checks, use the
[verification procedure](../local-verification.md). The authoritative trust-path
list is [managed-policy.json](../../verification/managed-policy.json); proposed
controls become authoritative only after merging to `main`.
