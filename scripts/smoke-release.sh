...
HOME="$isolated/home" XDG_CONFIG_HOME="$isolated/config" XDG_RUNTIME_DIR="$isolated/runtime" HERDR_SOCKET_PATH="$isolated/no-herdr.sock" HERDR_MISE_DEMO_COUNT=12 "$binary" >"$log" 2>&1 &
pid=$!
trap 'kill -INT "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true; rm -rf "$isolated"' EXIT INT TERM

i=0
until curl --fail --silent http://127.0.0.1:8686/ -o "$isolated/index.html" 2>/dev/null; do
  i=$((i + 1))
...
[ -n "$asset" ]
curl --fail --silent "http://127.0.0.1:8686/$asset" -o "$isolated/client.js"
node -e '
