...
if (!existsSync(manifestPath))
  throw new Error("Vite manifest missing; run the production build first.");
...
function include(item) {
  if (!item || loaded.has(item.file)) return;
  loaded.add(item.file);
  for (const dependency of item.imports ?? []) include(manifest[dependency]);
}
include(entry[1]);
...
const runtime = [...loaded]
  ...
  .map((file) => ({
    file,
    raw: statSync(join(dist, file)).size,
    gzip: gzipSync(readFileSync(join(dist, file))).length,
  }));
...
const totalTransfer = emitted.reduce(
