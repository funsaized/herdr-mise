...
test("Nightshift planning fan-out serializes canonical unique work items", () => {
  const source = nightshiftWorkflows["workflow-nightshift-plan-fanout.yaml"];
  assert.ok(source);
  assert.match(source, /minItems: 1/);
  assert.match(source, /maxItems: 100/);
  assert.match(source, /uniqueItems: true/);
  assert.ok(source.includes("pattern: '^[0-9]+$'"));
  ...
  assert.match(source, /workflowIdOrName: nightshift-plan/);
  assert.ok(source.includes("issueNumber: ${{ int(self.workItem) }}"));
  assert.match(source, /^\s+reviewFeedback: \$\{\{/m);
  assert.match(source, /^\s+currentPlan: \$\{\{/m);
  assert.match(source, /^\s+humanFeedback: '\$\{\{/m);
});
...
test("Nightshift parks fresh failed review rounds at cycle four", () => {
  for (const [id, next] of [
    ...
    const stage = nightshiftFactory.slice(
      nightshiftFactory.indexOf(`        - id: ${id}\n`),
      nightshiftFactory.indexOf(`        - id: ${next}\n`),
    );
  ...
  const parked = nightshiftFactory.slice(
    nightshiftFactory.indexOf("        - id: parked\n"),
    nightshiftFactory.indexOf("        - id: ship-prep\n"),
  );
  ...
  const globalTransitions = nightshiftFactory.slice(
    nightshiftFactory.indexOf("    globalTransitions:\n"),
  );
  assert.match(
    globalTransitions,
