...
if (!output)
  throw new Error(
    "usage: node scripts/generate-third-party-notices.mjs <output>",
  );
...
function section(kind, dependency, directory) {
  ...
  const fallback = fallbackLicenseFiles.get(
  ...
  if (!texts)
    throw new Error(
      `missing license text for ${dependency.name}@${dependency.version}`,
    );
  return `### ${kind}: ${header}\n\n${texts}`;
}
...
for (const target of releaseTargets) {
  ...
  const packages = new Map(
    cargo.packages.map((dependency) => [dependency.id, dependency]),
  );
  ...
  while (queue.length > 0) {
    ...
    for (const edge of node.deps) {
      if (edge.dep_kinds.some(({ kind }) => kind === null))
        queue.push(edge.pkg);
    }
  }
}
const rust = [...runtimePackages.values()]
  ...
  .map((dependency) =>
    section("Rust dependency", dependency, dirname(dependency.manifest_path)),
  );

const clientRoot = resolve("client");
const npmPaths = execFileSync(
  "npm",
  ["ls", "--prefix", "client", "--omit=dev", "--all", "--parseable"],
  {
    encoding: "utf8",
    maxBuffer: 20 * 1024 * 1024,
  },
...
const javascript = npmPaths
  .map((directory) => {
    const manifest = join(directory, "package.json");
    if (!existsSync(manifest))
      throw new Error(`missing package manifest: ${manifest}`);
    return {
      dependency: JSON.parse(readFileSync(manifest, "utf8")),
      directory,
    };
  })
  .sort((a, b) =>
    `${a.dependency.name}@${a.dependency.version}`.localeCompare(
      `${b.dependency.name}@${b.dependency.version}`,
    ),
  )
  .map(({ dependency, directory }) =>
    section("JavaScript dependency", dependency, directory),
  );
...
console.log(
  `wrote ${output} (${rust.length} Rust, ${javascript.length} JavaScript, ${fonts.length} font notices)`,
);
