...
test('compatibility manifest is the complete supported release authority', () => {
  ...
  assert.deepEqual(manifest.supported.map(({ release, protocol }) => ({ release, protocol })), [
    { release: '0.7.5', protocol: 17 },
    { release: '0.8.0', protocol: 19 },
  ]);
  assert.equal(new Set(manifest.supported.map(entry => entry.release)).size, manifest.supported.length);
  assert.equal(new Set(manifest.supported.map(entry => entry.protocol)).size, manifest.supported.length);
  for (const entry of manifest.supported) {
    assert.match(entry.upstreamCommit, /^[0-9a-f]{40}$/);
    assert.match(entry.fixture, /^server\/tests\/fixtures\/snapshot-herdr-[\w.-]+\.json$/);
...
test('public compatibility tables are generated exactly from the manifest', () => {
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  const expected = manifest.supported.map(entry => `| \`${entry.release}\` | \`${entry.protocol}\` |`).join('\n');
  for (const path of ['README.md', 'docs/operations.md']) {
    const document = readFileSync(path, 'utf8');
    const table = document.match(/<!-- herdr-compatibility:start -->[\s\S]*?\n([\s\S]*?)\n<!-- herdr-compatibility:end -->/)?.[1];
    assert.ok(table, `${path} compatibility markers`);
    assert.equal(table, `| Herdr release | Snapshot protocol |\n|---|---|\n${expected}`);
  }
});
...
test('adapter mapping audit rejects every manifest fixture absent from Rust cases', () => {
  ...
  const adapter = 'include_str!("../tests/fixtures/snapshot-herdr-0.7.5-p17.json")';
...
test('fixture privacy audit rejects obvious private material', () => {
  assert.deepEqual(auditFixture({ pane_id: 'fictional-pane', agent: 'example-agent' }), []);
  for (const unsafe of [
...
test('fixture privacy audit rejects real workspace labels and session values', () => {
  ...
  assert.ok(auditFixture({ agent_session: { value: 'session-42' } }).length > 0);
});

test('scheduled compatibility workflow is immutable, read-only, and non-publishing', () => {
  const workflow = readFileSync('.github/workflows/herdr-compatibility-drift.yml', 'utf8');
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
  assert.match(workflow, /schedule:/);
  assert.match(workflow, /workflow_dispatch:/);
  assert.match(workflow, /permissions:\n  contents: read/);
  for (const { upstreamCommit } of manifest.supported) assert.ok(workflow.includes(`ref: ${upstreamCommit}`));
...
test('workflow audit rejects privileged triggers and additional permission declarations', () => {
  const workflow = readFileSync('.github/workflows/herdr-compatibility-drift.yml', 'utf8');
  const privilegedTrigger = workflow.replace('  workflow_dispatch:', '  workflow_dispatch:\n  pull_request_target:');
  const jobPermissions = workflow.replace('  verify:\n', '  verify:\n    permissions:\n      contents: write\n');
  ...
  assert.ok(auditWorkflow(privilegedTrigger).includes('compatibility workflow uses pull_request_target'));
  assert.ok(auditWorkflow(jobPermissions).includes('compatibility workflow has additional permission declarations'));
  assert.ok(auditWorkflow(extraTopLevelPermission).includes('compatibility workflow permissions are not exactly contents: read'));
  assert.ok(auditWorkflow(blankSeparatedPermission).includes('compatibility workflow permissions are not exactly contents: read'));
});
...
test('upstream checker reads Cargo version, wire protocol, and SessionSnapshot schema from their real split sources', () => {
  ...
  try {
    const args = entries.map(entry => {
      ...
      writeFileSync(join(directory, 'src/protocol/wire.rs'), `pub const PROTOCOL_VERSION: u32 = ${entry.protocol};\n`);
      writeFileSync(join(directory, 'src/api/schema/session.rs'), `
