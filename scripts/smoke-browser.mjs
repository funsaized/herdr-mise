...
try {
  ...
  try {
    await page.getByText("DEMO SERVICE").waitFor({ state: "visible", timeout: 5000 });
  } catch (error) {
    console.error(`browser_body=${(await page.locator("body").innerText()).replaceAll("\n", " | ")}`);
    throw error;
  }
  ...
  if (elapsed > 1500) throw new Error(`page readiness ${elapsed.toFixed(1)} ms exceeds 1500 ms`);
