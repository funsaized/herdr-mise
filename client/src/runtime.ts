...
export interface ReducedMotionPreference { current(): boolean; subscribe(listener:(reduced:boolean)=>void):()=>void }
export function createReducedMotionPreference(query:MediaQueryList):ReducedMotionPreference {
  return {
    current:()=>query.matches,
    subscribe(listener){const handle=(event:MediaQueryListEvent)=>listener(event.matches);query.addEventListener("change",handle);return()=>query.removeEventListener("change",handle);},
  };
}
const motionQuery=()=>typeof matchMedia==="function"?matchMedia("(prefers-reduced-motion: reduce)"):{matches:false,addEventListener(){},removeEventListener(){}} as unknown as MediaQueryList;
...
export const reducedMotionPreference=createReducedMotionPreference(motionQuery());
