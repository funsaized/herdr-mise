...
export function isVisualMode(mode: string) {
  return mode === "visual";
}
...
export function parseVisualConfig(search: string): VisualConfig {
  const query = new URLSearchParams(search),
    ...
    count = Number(query.get("agents"));
  return {
    preset:
      preset !== null && presets.has(preset as VisualPreset)
        ? (preset as VisualPreset)
        : defaultVisualConfig.preset,
    agents:
...
const stateAgeMs: Record<Exclude<AgentState, "ended">, number> = {
  idle: 12_000,
  working: 18_000,
  blocked: 45_000,
  done: 8_000,
...
const identities = [
  ["Codex", "gpt-5.3-codex", "checkout-api"],
  ["Claude", "claude-sonnet-4", "payments"],
  ["Hermes", "hermes-4", "order-routing"],
  ["OpenClaw", "openclaw-2", "kitchen-ui"],
  ["Gemini", "gemini-2.5-pro", "inventory"],
  ["Aider", "deepseek-v3", "receipts"],
] as const;
const mixedStates = [
  "working",
  "blocked",
...
function agent(
  ...
  return {
    ...
    name: mixed
      ? `${identity[0]}${mixedSuffix}`
      : `mise-${String(index + 1).padStart(2, "0")}`,
    state,
    progress,
    stateEnteredAt: new Date(
      now - stateAgeMs[state] - (mixed ? index * 7_000 : 0),
    ).toISOString(),
    accentIndex: index % 12,
    model: mixed ? identity[1] : "codex",
    workspace: mixed
...
export function buildVisualFeed(
  config: VisualConfig,
  now = Date.now(),
): AgentStateEvent[] {
  ...
  return [
    ...
    ...agents.map((record) => ({
      ...
      agent: { ...record, state: "ended" as const },
    })),
  ];
}
...
export function installVisualWebSocket(
  ...
  class VisualWebSocket {
    ...
    close() {
      if (this.readyState === VisualWebSocket.CLOSED) return;
      this.readyState = VisualWebSocket.CLOSED;
      if (this.heartbeatTimer !== null)
        globalThis.clearInterval(this.heartbeatTimer);
      this.heartbeatTimer = null;
      for (const timer of this.lifecycleTimers) globalThis.clearTimeout(timer);
...
export function initializeVisualMode(
  mode: string,
  target: WebSocketTarget,
  search: string,
  production = false,
