...
const kitchen: SceneTheme = {
  ...
  animations: {
    idle: {
      frames: ["chop-0", "chop-1", "chop-2", "chop-3"],
      frameMs: 140,
      loop: true,
    },
    working: {
      frames: ["fire-0", "fire-1", "fire-2", "fire-1"],
      frameMs: 120,
      loop: true,
    },
    blocked: {
      frames: ["bell-0", "bell-1", "bell-2", "bell-1"],
      frameMs: 140,
      loop: true,
    },
    done: {
      frames: ["plate-0", "plate-1", "plate-2", "plate-3"],
      frameMs: 160,
      loop: false,
    },
    ended: {
      frames: ["exit-0", "exit-1", "exit-2", "exit-3"],
      frameMs: 150,
      loop: false,
    },
  },
...
export function registerTheme(theme: SceneTheme) {
  registry.set(theme.id, theme);
}
export function getTheme(id = "kitchen") {
...
export function paletteIndex(theme: ResolvedTheme) {
  return theme === "dark" ? 1 : 0;
}
export function accentIndexForId(id: string) {
  let value = 2166136261;
  for (let index = 0; index < id.length; index++) {
    value ^= id.charCodeAt(index);
    value = Math.imul(value, 16777619);
  }
  return (value >>> 0) % tokens.accents.length;
}
