import type { AgentRecord, AgentStateEvent, AppMode as FeedMode } from "../../../protocol/generated/agent-state-event";
...
export interface StoreSnapshot { agents: ReadonlyMap<string, AgentMachine>; board: readonly BoardEntry[]; mode: AppMode; feedMode: FeedMode; selectedId: string | null; settings: Settings; lastUpdateAt: number }
export interface CoarseSlice { count: number; blocked: number; done: number; mode: AppMode; selectedId: string | null; settings: Settings }
...
export class AgentStore {
  ...
  snapshot(): StoreSnapshot { return { agents: this.agents, board: this.board, mode: this.mode, feedMode: this.feedMode, selectedId: this.selectedId, settings: this.settings, lastUpdateAt: this.lastUpdateAt }; }
  coarse(): CoarseSlice { const values = [...this.agents.values()]; return { count: values.length, blocked: values.filter(a => a.targetState === "blocked").length, done: values.filter(a => a.targetState === "done").length, mode: this.mode, selectedId: this.selectedId, settings: this.settings }; }
...
function sameCoarse(a: CoarseSlice, b: CoarseSlice) { return a.count === b.count && a.blocked === b.blocked && a.done === b.done && a.mode === b.mode && a.selectedId === b.selectedId && a.settings === b.settings; }
