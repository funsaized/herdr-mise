...
describe("visual harness configuration", () => {
  ...
  it("constructs stable protocol feeds for all supported counts and active presets", () => {
    for (const agents of [1, 2, 6, 12] as const) for (const preset of ["idle", "working", "blocked", "done"] as const) {
      const feed = buildVisualFeed({ preset, agents, theme: "light" });
      expect(feed).toHaveLength(1);
      expect(feed[0]).toMatchObject({ version: 1, type: "snapshot", mode: "demo" });
      if (feed[0]?.type === "snapshot") {
        expect(feed[0].agents).toHaveLength(agents);
        expect(feed[0].agents.every(agent => agent.state === preset)).toBe(true);
        expect(feed[0].agents.map(agent => agent.progress)).toEqual(preset === "working" ? Array.from({ length: agents }, (_, index) => (index + 1) / 13) : Array(agents).fill(null));
      }
    }
  });

  it("keeps every emitted progress value within protocol bounds", () => {
    for (const agents of [1, 2, 6, 12] as const) for (const preset of ["idle", "working", "blocked", "done", "ended", "mixed"] as const) {
      for (const event of buildVisualFeed({ preset, agents, theme: "light" })) {
        const records = event.type === "snapshot" ? event.agents : event.type === "delta" && event.operation === "upsert" ? [event.agent] : [];
        expect(records.every(agent => agent.progress === null || (agent.progress >= 0 && agent.progress <= 1))).toBe(true);
      }
    }
  });
  ...
  it("builds an inspectable mixed service without changing existing counts", () => {
    ...
    const [event] = buildVisualFeed({ preset: "mixed", agents: 6, theme: "light" }, now);
    expect(event?.type).toBe("snapshot");
    if (event?.type !== "snapshot") return;
    expect(event.agents).toHaveLength(6);
    expect(new Set(event.agents.map(agent => agent.state)).size).toBeGreaterThanOrEqual(3);
    expect(event.agents.some((agent, index) => event.agents.findIndex(other => other.state === agent.state) !== index)).toBe(true);
    expect(new Set(event.agents.map(agent => agent.model)).size).toBeGreaterThanOrEqual(4);
    expect(new Set(event.agents.map(agent => agent.workspace)).size).toBe(6);
    expect(new Set(event.agents.map(agent => agent.accentIndex)).size).toBe(6);
    expect(event.agents.some(agent => agent.state === "blocked" && now - Date.parse(agent.stateEnteredAt) >= 45_000)).toBe(true);
  });

  it("keeps all twelve mixed visible identities and workspaces unique", () => {
    const [event] = buildVisualFeed({ preset: "mixed", agents: 12, theme: "light" });
    expect(event?.type).toBe("snapshot");
    if (event?.type !== "snapshot") return;
    expect(new Set(event.agents.map(agent => `${agent.name}:${agent.workspace}`))).toHaveLength(12);
    expect(new Set(event.agents.map(agent => agent.workspace))).toHaveLength(12);
  });

  it("anchors every preset's active record at a deterministic recent age", () => {
    const now = Date.parse("2026-08-01T15:00:00.000Z");
    const expected = { idle: 12_000, working: 18_000, blocked: 45_000, done: 8_000, ended: 8_000 } as const;
    for (const preset of ["idle", "working", "blocked", "done", "ended"] as const) {
      const [event] = buildVisualFeed({ preset, agents: 1, theme: "light" }, now);
      expect(event?.type).toBe("snapshot");
      if (event?.type === "snapshot") expect(now - Date.parse(event.agents[0]!.stateEnteredAt)).toBe(expected[preset]);
    }
  });

  it("models ended through prior active truth and ended deltas", () => {
    const store = new AgentStore();
    for (const event of buildVisualFeed({ preset: "ended", agents: 2, theme: "light" })) store.apply(event);
    expect(store.snapshot().agents.size).toBe(0);
    expect(store.snapshot().board).toHaveLength(2);
    expect(store.snapshot().board.every(entry => entry.finalState === "done")).toBe(true);
  });
});
...
describe("visual WebSocket boundary", () => {
  it("moves the mixed-feed hero through working, blocked, answered, working, and done", async () => {
    ...
    installVisualWebSocket(target, { preset: "mixed", agents: 6, theme: "light" });
    const socket = new target.WebSocket!("ws://visual/ws");
    const messages: unknown[] = [];
    socket.onmessage = event => messages.push(JSON.parse(String(event.data)));
    await vi.runAllTicks();
    const heroEvents = () => messages.flatMap(event => {
      if (!event || typeof event !== "object") return [];
      const message = event as { type?: string; agents?: Array<{ id: string; state: string }>; agent?: { id: string; state: string } };
      if (message.type === "snapshot") return message.agents?.filter(agent => agent.id === "visual-agent-1").map(agent => agent.state) ?? [];
      return message.agent?.id === "visual-agent-1" ? [message.agent.state] : [];
    });
    ...
    const heroRecords = messages.flatMap(event => {
      if (!event || typeof event !== "object") return [];
      const message = event as { type?: string; agents?: Array<{ id: string; name: string; accentIndex: number }>; agent?: { id: string; name: string; accentIndex: number } };
      if (message.type === "snapshot") return message.agents?.filter(agent => agent.id === "visual-agent-1") ?? [];
      return message.agent?.id === "visual-agent-1" ? [message.agent] : [];
    });
    expect(heroRecords).toHaveLength(4);
    expect(heroRecords.every(record => record.id === "visual-agent-1" && record.name === "Codex" && record.accentIndex === 0)).toBe(true);
    socket.close();
  });
  ...
  it("shows a generic temporary answer cue only after blocked returns to working", () => {
    ...
    expect(stationIdentityLabels(agent, "working", now + 2_001).status).toContain("FIRE");
  });
  ...
  it("delivers the deterministic feed through AgentWebSocketClient", async () => {
    const target = {} as { WebSocket?: typeof WebSocket };
    installVisualWebSocket(target, { preset: "blocked", agents: 2, theme: "light" });
    const store = new AgentStore();
    const client = new AgentWebSocketClient("ws://visual/ws", store, url => new target.WebSocket!(url) as never);
    client.start();
    await new Promise(resolve => setTimeout(resolve, 0));
    expect(store.snapshot().mode).toBe("demo");
    expect([...store.snapshot().agents.values()].map(agent => agent.state)).toEqual(["blocked", "blocked"]);
    client.stop();
  });

  it("delegates every socket except the application /ws endpoint", () => {
    const nativeSocket = { readyState: WebSocket.CONNECTING };
    const Native = vi.fn(function () { return nativeSocket; }) as unknown as typeof WebSocket;
    const target = { WebSocket: Native };
    installVisualWebSocket(target, { preset: "working", agents: 1, theme: "light" });
    const Installed = target.WebSocket!;
    expect(new Installed("ws://localhost:5173/@vite/client")).toBe(nativeSocket);
  ...
  it("stays live beyond the stale window and releases heartbeat timers on close", async () => {
    ...
    const target = {} as { WebSocket?: typeof WebSocket };
    installVisualWebSocket(target, { preset: "working", agents: 1, theme: "light" });
    const store = new AgentStore(), disconnected = vi.spyOn(store, "setDisconnected"), client = new AgentWebSocketClient("ws://visual/ws", store, url => new target.WebSocket!(url) as never);
    client.start();
    await vi.runAllTicks();
    expect(store.snapshot().mode).toBe("demo");
  ...
  it("can close while CONNECTING without opening or leaking timers", async () => {
    ...
    installVisualWebSocket(target, { preset: "working", agents: 1, theme: "light" });
    const socket = new target.WebSocket!("ws://visual/ws");
    const opened = vi.fn(), messaged = vi.fn();
  ...
  it("isolates visual settings from persisted production settings", () => {
    const storage = { getItem: vi.fn(() => JSON.stringify({ version: 1, settings: { theme: "dark" } })), setItem: vi.fn() };
  ...
  it("uses the visible production done timeout without persisted settings", () => {
    ...
    for (const event of buildVisualFeed({ preset: "done", agents: 1, theme: "light" })) visual.apply(event);
  ...
  it("installs the mock only in visual startup mode", () => {
    const native = vi.fn() as unknown as typeof WebSocket, target = { WebSocket: native };
    expect(initializeVisualMode("production", target, "?preset=done")).toBeNull();
    expect(target.WebSocket).toBe(native);
    expect(initializeVisualMode("visual", target, "?preset=done")?.preset).toBe("done");
