...
export class KitchenScene {
  ...
  reveal(id: string) {
    ...
    const ids = [...this.store.snapshot().agents.keys()],
  ...
  private setPage(page: number) {
    ...
    const layout = computeLayout(
      this.host.clientWidth || innerWidth,
      this.host.clientHeight || innerHeight,
      [...this.store.snapshot().agents.keys()],
      page,
    );
  ...
  private redraw() {
    ...
    const ids = [...snapshot.agents.keys()],
      liveIds = ids.join("|"),
      rosterChanged = liveIds !== this.lastLiveIds;
    this.lastLiveIds = liveIds;
    ...
    const width = this.host.clientWidth || innerWidth,
      height = this.host.clientHeight || innerHeight;
    let nextLayout = computeLayout(width, height, ids, this.requestedPage);
    const anchor = [this.focusedId, snapshot.selectedId].find(
    ...
    if (rosterChanged && anchor && !nextLayout.visibleIds.includes(anchor))
      nextLayout = computeLayout(
        ...
        height,
        ids,
        Math.floor(ids.indexOf(anchor) / nextLayout.capacity),
      );
  ...
  private drawStations(now: number) {
    ...
    const blockedIds = [...snapshot.agents.values()]
